...
GCP_PATH_TEMPLATE = 'raw/green_tripdata/{{ execution_date.strftime(\'%Y\') }}/green_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.parquet'
...
with gcp_green:
    ...
    wget_task = BashOperator(
        task_id='wget',
        bash_command=f'curl -sSL {URL_TEMPLATE} > {PARQUET_OUTPUT_FILE_TEMPLATE}'
    )
    ...
    del_data_task = BashOperator(
        task_id="del_data",
        bash_command=f"rm {PARQUET_OUTPUT_FILE_TEMPLATE}",
    )

    # Dependencies, basically states what order task have to be completed in
    wget_task >> local_to_gcs_task >> del_data_task
